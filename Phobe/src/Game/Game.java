...
public class Game extends JComponent implements Runnable {
	...
	public synchronized void Update() {
		...
		if (!onePlayerAlive || gameTimeExpired) {
			...
			Robot winnerBot = this.getRobots().get(0);
			
			for (int i = 1; i < this.getRobots().size(); i++) {
				if(winnerBot.getDistance() < this.getRobots().get(i).getDistance())
					winnerBot = this.getRobots().get(i);
			}
			int winnerCount = 0;
			
			for (Robot rob : this.getRobots()) {
				if(winnerBot.getDistance() == rob.getDistance())
					winnerCount++;
			}

			for (GameEndedListener gEndLis : gameEndedListeners) {
				gEndLis.GameEnded((winnerCount == 1)? winnerBot : null );
			}
		}
